...
describe('Registration: /fi/helsinki/ with Visa, with business account', function () {
    ...
    it("Enter Code / Mobile Number negative", async function () {
        await regPage.enterPhone("invalid","939177068")
    ...
    it("Enter Code / Mobile Number positive", async function () {
        await regPage.enterPhone("00380","939177068");
    ...
    it("Date of Birth", async function () {
        await regPage.selectDateOfBirth(9)
        await expect(regElm.day.getAttribute("value")).toEqual('9');
        await regPage.selectMonthOfBirth(12);
        await expect(regElm.month.getAttribute("value")).toEqual('12');
        await regPage.selectYearOfBirth(28);
        await expect(regElm.year.getAttribute("value")).toEqual('1990');
    ...
    it("Add business address checkbox", async function () {
        await regPage.clickAddBusinessAddressCheckbox();
        await expect(helper.isDisplayed(regElm.business_companyNameField)).toEqual(true)
    ...
    it("Company Name Field", async function () {
        await regPage.addCompanyName("SpaceX");
        await expect(regElm.business_companyNameField.getAttribute("value")).toEqual("SpaceX");
    });

    it("Company Street / Number", async function () {
        await regPage.addCompanyStreetDetails("business street","22");
        await expect(regElm.business_street_field.getAttribute("value")).toEqual("business street");
        await expect(regElm.business_streetNumber_field.getAttribute("value")).toEqual("22");
    });

    it("Company PostalCode",async function () {
        await regPage.addCompanyPostalCode("01100");
        await expect(regElm.business_postalCode_field.getAttribute("value")).toEqual("01100")
    });

    it("Company City", async function () {
        await regPage.addCompanyCity("Houston");
        await expect(regElm.business_City_field.getAttribute("value")).toEqual("Houston")
    });

    it("Select Company Country from dropdown", async function () {
        await regPage.selectCompanyCountry(1);
        await expect(regElm.business_country_dropdown.getAttribute("value")).toEqual("FI")
    });

    it("NEXT BUTTON", async function () {
        await regPage.clickNext();
        await expect(browser.isElementPresent(regElm.licence_0)).toEqual(true);
        await expect(regElm.dl_info_text.getText()).toEqual("*Uudessa EU-ajokortissa ajokorttisi numero löytyy kohdasta" +
            " 5 (yleensä sama kuin henkilötunnuksesi)." +
            " Mikäli sinulla on vanhempi ajokortti," +
            " syötä tällöin henkilötunnuksesi (sekä alku- että loppuosa).");
    });

    //>>>THIRD REG PAGE<<<<

    it("Driving Licence field", async function () {
        await regPage.enterDrivingLicence("123456qwe89");
    ...
    it("Select DriverLicence Country from drop-down picker", async function () {
        await regPage.selectLicenceCountry(0);
        await expect(regElm.lCountry.getAttribute("value")).toEqual("FI")

    });

    it("DL expirience should be not less than 1 year",async function () {
        await regPage.licenceValidFrom(4,9,1);
        await regPage.clickNext();
        await expect(regElm.dl_validFrom_error.getText()).toEqual("Valitse myöntämispäivä")
    });

    it("DL Valid From field positive",async function () {
        await regPage.licenceValidFrom(4,9,2);
        await expect(browser.isElementPresent(regElm.dl_validFrom_error)).toEqual(false);
    });
